...
- **客户端软件更新**：正式版启动后自动检查，也可从顶栏手动检查；发现新版本后在客户端内下载，完成后重启安装。
- **平台边界**：Windows 使用 NSIS 更新；macOS 同时发布 DMG 和 ZIP，并要求 Developer ID 签名。未签名的 macOS 构建会停用客户端更新。
...
- [x] **V2.4.1 客户端更新**：启动检查、手动检查、下载进度、重启安装、单例监听和发布元数据。
