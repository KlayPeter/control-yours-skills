  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.15fr,0.85fr]">
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <div className="app-card p-4">
                <p className="text-sm font-medium app-text">{t.sectionLocalInstall}</p>
                <p className="mt-2 text-sm app-text-soft">
                  {t.overviewPageCenterBody || "看主版本、做分类、绑定同步目标。"}
                </p>
              </div>
              <div className="app-card p-4">
                <p className="text-sm font-medium app-text">{t.sectionProjects}</p>
                <p className="mt-2 text-sm app-text-soft">
                  {t.overviewPageProjectsBody || "接入你自己的项目，扫描里面已有的 Skill。"}
                </p>
              </div>
              <div className="app-card p-4">
                <p className="text-sm font-medium app-text">{t.sectionStaged}</p>
                <p className="mt-2 text-sm app-text-soft">
                  {t.overviewPageStagedBody || "导入后的来源先在这里检查，再决定是否安装。"}
                </p>
              </div>
              <div className="app-card p-4">
                <p className="text-sm font-medium app-text">{t.sectionSyncStatus}</p>
                <p className="mt-2 text-sm app-text-soft">
                  {t.overviewPageSyncBody || "查看待同步、失败和冲突，决定下一步怎么分发。"}
                </p>
              </div>
              <div className="app-card p-4">
                <p className="text-sm font-medium app-text">{t.sectionAiWorkspace}</p>
                <p className="mt-2 text-sm app-text-soft">
                  {t.overviewPageAiBody || "查看这台电脑上 Codex、Claude、Agents 已有的 Skill 来源。"}
                </p>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <OverviewMetric label={t.overviewMetricCenterRepository || "中心仓库技能"} value={centerRepositoryCount} />
          <OverviewMetric label={t.overviewMetricUncategorized || "未分类技能"} value={uncategorizedCount} />
          <OverviewMetric label={t.overviewMetricConnectedProjects || "已接入项目"} value={snapshot.importedProjects.length} />
        </div>
      </SectionCard>
