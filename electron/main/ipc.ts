...
export function registerIpcHandlers(backend: SkillManagerBackend) {
  ...
  ipcMain.handle("skill-manager:create-workspace-folder", (_event, input) =>
  ...
  ipcMain.handle("skill-manager:add-sync-target", (_event, input) =>
    backend.addSyncTarget(input)
  );
  ipcMain.handle("skill-manager:remove-sync-target", (_event, input) =>
    backend.removeSyncTarget(input)
  );
  ipcMain.handle("skill-manager:sync-installed-skill", (_event, input) =>
